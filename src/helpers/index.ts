...
const insertTextInActiveTextEditor = (text: string, position: vscode.Position) => {
...
const suggestCodeFromComment = async (gptWebViewProvider: vscode.WebviewViewProvider, statusBarItem: vscode.StatusBarItem, apiKey: string, language: string, comment: string | undefined, insertSuggestionAt: number | undefined) => {
    if (comment?.trim() === '') {
    ...
    const showProgressNotification = () => {
        vscode.window.withProgress({
            ...
            try {
                const query: string = `suggest just code, no comments in ${language} to ${comment} `;
...
const getSuggestions = async (suggestionType: string, gptWebViewProvider: vscode.WebviewViewProvider, statusBarItem: vscode.StatusBarItem, apiKey: string, language: string, progressMessage: string) => {
    const editor = vscode.window.activeTextEditor;
